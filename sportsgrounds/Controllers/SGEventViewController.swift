...
class SGEventViewController: SGViewController {
    ...
    private lazy var tableView: SGSelfSizingTableView = {
        ...
        tableView.register(SGPlaceholderCell.self, forCellReuseIdentifier: SGPlaceholderCell.reuseIdentifier)
    ...
    private func updateNavigationBar(withEvent event: SGEvent) {
        ...
        case .scheduled, .processing:
            ...
            if event.owner.id == user.user.id, editBarButtonItem == nil {
                let editBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "event.icon.edit"),
                                                        style: .plain,
                                                        target: self,
                                                        action: #selector(editTouchUpInside(_:)))
                editBarButtonItem.imageInsets = UIEdgeInsets(top: -1, left: 0, bottom: 1, right: 0)
                editBarButtonItem.tintColor = UIColor.appBlack
                self.editBarButtonItem = editBarButtonItem
                
                rightBarButtonItems.append(editBarButtonItem)
            }
            ...
            if leaveBarButtonItem == nil {
                ...
                rightBarButtonItems.append(leaveBarButtonItem)
            }
        ...
        default:
            let rightBarButtonItems = self.navigationItem.rightBarButtonItems?.filter {
                $0 != self.leaveBarButtonItem && $0 != self.editBarButtonItem && $0 != self.chatBarButtonItem
            }
...
extension SGEventViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case let .placeholder(text, image):
            if let cell = tableView.dequeueReusableCell(withIdentifier: SGPlaceholderCell.reuseIdentifier,
                                                        for: indexPath) as? SGPlaceholderCell {
                cell.configure(withText: text, placeholder: image)
                return cell
            }
        }
...
extension SGEventViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case let .placeholder(text, _):
            return SGPlaceholderCell.height(forText: text, width: self.view.width)
        }
    }
