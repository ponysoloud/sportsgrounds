...
extension UIAlertController {
    ...
    public func show(animated: Bool = true, vibrate: Bool = false, style: UIBlurEffect.Style? = nil, completion: (() -> Void)? = nil) {
        ...
        DispatchQueue.main.async {
            UIApplication.shared.keyWindow?.visibleViewController?.present(self, animated: animated, completion: completion)
            if vibrate {
...
extension UIAlertController {
    ...
    func addEnumerationPicker<T: CaseIterable & SGCaseNameable>(_ enumType: T.Type, selectionHandler: @escaping (T) -> Void) {
        for c in T.allCases {
            self.addAction(title: c.title, style: .default, handler: {
                _ in
                selectionHandler(c)
            })
        }
